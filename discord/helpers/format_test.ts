...
Deno.test("discordTimestamp: epoch 0 returns valid tag", () => {
  assertEquals(discordTimestamp(0), "<t:0:R>");
});

Deno.test("discordTimestamp: 0 seconds relative format", () => {
  assertEquals(discordTimestamp(0, "F"), "<t:0:F>");
  assertEquals(discordTimestamp(0, "d"), "<t:0:d>");
});

Deno.test("formatPermissionInfo: negative number in roles still formats", () => {
  const result = formatPermissionInfo({ allowedRoles: ["-1"] });
  assertEquals(result, " (roles: <@&-1>)");
});
