...
Deno.test("embed-builder: exactly 25 fields accepted", () => {
  const builder = new EmbedBuilder();
  for (let i = 0; i < 25; i++) {
    builder.field(`Field ${i}`, `Value ${i}`);
  }
  const result = builder.build();
  assertEquals(result.fields?.length, 25);
  assertEquals(result.fields?.[0].name, "Field 0");
  assertEquals(result.fields?.[24].name, "Field 24");
});

Deno.test("embed-builder: empty build returns minimal object", () => {
  const result = new EmbedBuilder().build();
  assertEquals(result, {});
});

Deno.test("embed-builder: method chaining returns same instance", () => {
  const builder = new EmbedBuilder();
  const afterTitle = builder.title("T");
  const afterDesc = afterTitle.description("D");
  // Verify chaining returns the same builder instance
  assertEquals(afterTitle === builder, true);
  assertEquals(afterDesc === builder, true);
  assertEquals(afterDesc === afterTitle, true);
  // Verify both properties are set on the single instance
  const result = builder.build();
  assertEquals(result.title, "T");
  assertEquals(result.description, "D");
});
