...
Deno.test("url: protocol-less URL is rejected", () => {
  assertEquals(isValidPublicUrl("example.com"), false);
  assertEquals(isValidPublicUrl("www.example.com/path"), false);
});

Deno.test("url: javascript: protocol is rejected", () => {
  assertEquals(isValidPublicUrl("javascript:alert(1)"), false);
});

Deno.test("url: very long URL is still valid if HTTPS", () => {
  const longPath = "a".repeat(2000);
  assertEquals(isValidPublicUrl(`https://example.com/${longPath}`), true);
});
