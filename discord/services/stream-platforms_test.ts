...
import { checkTwitchStreams, checkYouTubeStream, checkKickStream, _internals } from "./stream-platforms.ts";
...
Deno.test("twitch: resetTokenCache clears cache forcing new token fetch", async () => {
  Deno.env.set("TWITCH_CLIENT_ID", "test_id");
  Deno.env.set("TWITCH_CLIENT_SECRET", "test_secret");
  _internals.resetTokenCache();
  mockFetch({
    responses: [
      // Token request
      { status: 200, body: { access_token: "fresh_tok", expires_in: 3600 } },
      // Streams API
      { status: 200, body: { data: [] } },
    ],
  });
  try {
    await checkTwitchStreams(["anyuser"]);
    const calls = getCalls();
    assertEquals(calls.length, 2);
    assert(calls[0].url.includes("oauth2/token"));
    assert(calls[1].url.includes("helix/streams"));
  } finally {
    restoreFetch();
    Deno.env.delete("TWITCH_CLIENT_ID");
    Deno.env.delete("TWITCH_CLIENT_SECRET");
...
Deno.test("twitch: expired token cache triggers new fetch", async () => {
  Deno.env.set("TWITCH_CLIENT_ID", "test_id");
  Deno.env.set("TWITCH_CLIENT_SECRET", "test_secret");
  _internals.resetTokenCache();
  // First call: get a token with a very short expiry
  mockFetch({
    responses: [
      // Token with 0s expiry (already expired by the time we use it again)
      { status: 200, body: { access_token: "short_tok", expires_in: 0 } },
      { status: 200, body: { data: [] } },
    ],
  });
  try {
    await checkTwitchStreams(["user1"]);
  } finally {
    restoreFetch();
  }
  // Second call: should fetch a new token since the old one is expired
  mockFetch({
    responses: [
      { status: 200, body: { access_token: "new_tok", expires_in: 3600 } },
      { status: 200, body: { data: [] } },
    ],
  });
  try {
    await checkTwitchStreams(["user2"]);
    const calls = getCalls();
    assertEquals(calls.length, 2);
    assert(calls[0].url.includes("oauth2/token"));
  } finally {
...
Deno.test("twitch: token request failure throws", async () => {
  Deno.env.set("TWITCH_CLIENT_ID", "test_id");
  Deno.env.set("TWITCH_CLIENT_SECRET", "test_secret");
  _internals.resetTokenCache();
  mockFetch({
    default: { status: 401, body: "Unauthorized" },
  });
  try {
    await assertRejects(
      () => checkTwitchStreams(["user"]),
