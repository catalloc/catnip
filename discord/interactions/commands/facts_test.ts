...
Deno.test("facts: negative page modulo wraps correctly", () => {
  const { embed } = buildFactPage(-3);
  const expectedIdx = (((-3) % FACTS.length) + FACTS.length) % FACTS.length;
  assertEquals(embed.description, FACTS[expectedIdx]);
  assertEquals(embed.title, `Fact ${expectedIdx + 1} of ${FACTS.length}`);
});

Deno.test("facts: page beyond length wraps to 0", () => {
  const { embed } = buildFactPage(FACTS.length);
  assertEquals(embed.description, FACTS[0]);
  assertEquals(embed.title, `Fact 1 of ${FACTS.length}`);
});
