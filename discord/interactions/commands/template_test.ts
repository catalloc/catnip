...
Deno.test("template: 25-field boundary allows exactly 25 fields", async () => {
  resetStore();
  const fields = Array.from({ length: 24 }, (_, i) => ({ name: `f${i}`, value: `v${i}` }));
  await blob.setJSON("template:g1:test", makeTemplate({ fields }));
  const mod = (await import("./template.ts")).default;
  const result = await mod.execute({
    guildId: "g1",
    userId: "u1",
    options: { subcommand: "add-field", name: "test", "field-name": "f24", "field-value": "v24" },
    memberRoles: [],
    memberPermissions: ADMIN_PERMISSIONS,
  } as any);
  assertEquals(result.success, true);
  ...
  const entry = await blob.getJSON<TemplateEntry>("template:g1:test");
  assertEquals(entry?.fields?.length, 25);
  assertEquals(entry?.fields?.[24].name, "f24");
});

Deno.test("template: color hex parsing accepts valid 6-digit hex", async () => {
  resetStore();
  await blob.setJSON("template:g1:test", makeTemplate({ color: 0xff0000 }));
  const mod = (await import("./template.ts")).default;
  const result = await mod.execute({
    guildId: "g1",
    userId: "u1",
...
Deno.test("template: sanitizeName empty string returns fallback", () => {
  const result = _internals.sanitizeName("");
  assertEquals(result, "");
  assertEquals(result.length, 0);
});

Deno.test("template: admin can specify channel option in send", async () => {
  resetStore();
  mockFetch({ default: { status: 200, body: { id: "msg1" } } });
  try {
    await blob.setJSON("template:g1:test", makeTemplate());
    ...
    const result = await mod.execute({
      guildId: "g1",
      userId: "u1",
      options: { subcommand: "send", name: "test", channel: "target-ch", channelId: "current-ch" },
      memberRoles: [],
      memberPermissions: ADMIN_PERMISSIONS,
    } as any);
    assertEquals(result.success, true);
    const calls = getCalls();
    // Admin channel option should be respected
    assert(calls.some((c) => c.url.includes("channels/target-ch/messages")));
  } finally {
    restoreFetch();
  }
});

Deno.test("template: create enforces max 25 templates per guild", async () => {
  resetStore();
  for (let i = 0; i < 25; i++) {
    await blob.setJSON(`template:g1:tmpl${i}`, makeTemplate());
  }
  ...
  const result = await mod.execute({
    guildId: "g1",
    userId: "u1",
    options: { subcommand: "create", name: "overflow" },
    memberRoles: [],
    memberPermissions: ADMIN_PERMISSIONS,
  } as any);
  assertEquals(result.success, false);
  assert(result.error?.includes("Maximum"));
  // Verify exactly 25 remain (none added)
  const existing = await blob.list("template:g1:");
  assertEquals(existing.length, 25);
});

Deno.test("template: preview with all fields populated shows complete embed", async () => {
  resetStore();
  await blob.setJSON("template:g1:full", makeTemplate({
    title: "Full Template",
  ...
  const result = await mod.execute({
    guildId: "g1",
    userId: "u1",
    options: { subcommand: "preview", name: "full" },
    memberRoles: [],
  } as any);
  assertEquals(result.success, true);
  assertEquals(result.embed?.title, "Full Template");
  assertEquals(result.embed?.description, "Complete description");
  assertEquals(result.embed?.color, 0x5865f2);
  assertEquals(result.embed?.footer?.text, "Footer text");
  assertEquals(result.embed?.image?.url, "https://example.com/image.png");
  assertEquals(result.embed?.fields?.length, 2);
  assertEquals(result.embed?.fields?.[0].name, "Field1");
  assertEquals(result.embed?.fields?.[0].inline, true);
  assertEquals(result.embed?.fields?.[1].name, "Field2");
  assertEquals(result.embed?.fields?.[1].inline, false);
});
