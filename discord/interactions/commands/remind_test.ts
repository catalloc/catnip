...
Deno.test("remind: duration 0s returns error", async () => {
  resetStore();
  const mod = (await import("./remind.ts")).default;
  const result = await mod.execute({
    guildId: "g1",
    userId: "u_0s",
    options: { duration: "0s", message: "immediate", channelId: "c1" },
    memberRoles: [],
  } as any);
  assertEquals(result.success, false);
  assert(result.error?.includes("Invalid duration"));
...
Deno.test("remind: message stored verbatim without @everyone expansion", async () => {
  resetStore();
  const mod = (await import("./remind.ts")).default;
  const result = await mod.execute({
    guildId: "g1",
    userId: "u_mention",
    options: { duration: "1h", message: "Hey @everyone check this", channelId: "c1" },
    memberRoles: [],
  } as any);
  assertEquals(result.success, true);

  const entries = await kv.list("reminder:u_mention:");
  assertEquals(entries.length, 1);
  const reminder = entries[0].value as Reminder;
  // Message is stored as-is (sanitization happens at delivery time, not storage)
  assertEquals(reminder.message, "Hey @everyone check this");
});
