...
Deno.test("counter: concurrent increment produces sequential values", async () => {
  resetStore();
  const r1 = await command.execute({ guildId: "g_seq", options: {} } as any);
  const r2 = await command.execute({ guildId: "g_seq", options: {} } as any);
...
Deno.test("counter: unknown action defaults to increment", async () => {
  resetStore();
  const r1 = await command.execute({ guildId: "g_unk", options: { action: "invalid" } } as any);
  assertEquals(r1.success, true);
  assertEquals(r1.message, "Counter: **1**");
  const r2 = await command.execute({ guildId: "g_unk", options: { action: "foobar" } } as any);
  assertEquals(r2.success, true);
  assertEquals(r2.message, "Counter: **2**");
});
