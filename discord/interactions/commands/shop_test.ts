...
Deno.test("shop: add with price 0 is rejected", async () => {
  resetStore();
  const result = await command.execute(ctx("add", { name: "Free", price: 0, description: "free item", type: "custom" }));
  assertEquals(result.success, false);
  assertStringIncludes(result.error!, "at least 1");
});

Deno.test("shop: add with negative price is rejected", async () => {
  resetStore();
  const result = await command.execute(ctx("add", { name: "NegPrice", price: -5, description: "negative price", type: "custom" }));
  assertEquals(result.success, false);
  assertStringIncludes(result.error!, "at least 1");
});

Deno.test("shop: buy with item number exceeding list length returns error", async () => {
  resetStore();
  await shop.addItem(guildId, { name: "OnlyItem", price: 10, description: "sole item", type: "custom" });
  const result = await command.execute(ctx("buy", { item: 5 }));
  assertEquals(result.success, false);
  assertStringIncludes(result.error!, "Invalid item number");
});
