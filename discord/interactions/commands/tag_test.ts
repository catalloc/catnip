...
Deno.test("tag: MAX_TAGS=50 limit enforced", async () => {
  resetStore();
  const tags: Record<string, any> = {};
  for (let i = 0; i < 50; i++) {
    tags[`tag${i}`] = makeTag({ content: `content${i}` });
  }
  await kv.set("tags:g_50", tags);

  const mod = (await import("./tag.ts")).default;
  const result = await mod.execute({
    guildId: "g_50",
    userId: "u1",
    options: { subcommand: "add", name: "tag50", content: "overflow" },
    memberRoles: [],
    memberPermissions: ADMIN_PERMISSIONS,
  } as any);
  assertEquals(result.success, false);
  assert(result.error?.includes("Maximum") || result.error?.includes("50") || result.error?.includes("limit"));
});

Deno.test("tag: sanitizeTagName strips backticks and markdown chars", () => {
  const result = _internals.sanitizeTagName("hello`world*test");
  assertEquals(result, "helloworldtest");
});

Deno.test("tag: sanitizeTagName truncates to 64 chars", () => {
  const longName = "a".repeat(100);
  const result = _internals.sanitizeTagName(longName);
  assertEquals(result.length, 64);
  assertEquals(result, "a".repeat(64));
});

Deno.test("tag: sanitizeTagName empty name returns empty string", () => {
  const result = _internals.sanitizeTagName("");
  assertEquals(result, "");
