...
Deno.test("poll: whitespace-only options are rejected", async () => {
  resetStore();
  const result = await pollCommand.execute({
    guildId: "g_ws",
    userId: "u1",
...
Deno.test("poll: zero votes displays correctly", () => {
  const config: PollConfig = {
  ...
  const embed = buildPollEmbed(config);
  assert(embed.description!.includes("Total votes: 0"));
  // Each option should show 0 votes
  assert(embed.description!.includes("0"));
});

Deno.test("poll: 5 options creates single row", () => {
  const opts = ["A", "B", "C", "D", "E"];
  const rows = buildPollComponents("g_5", opts);
  assertEquals(rows.length, 1);
  assertEquals(rows[0].components.length, 5);
});

Deno.test("poll: 6 options creates two rows", () => {
  const opts = ["A", "B", "C", "D", "E", "F"];
  const rows = buildPollComponents("g_6", opts);
  assertEquals(rows.length, 2);
  assertEquals(rows[0].components.length, 5);
  assertEquals(rows[1].components.length, 1);
});

Deno.test("poll: 10 options creates two rows", () => {
  const opts = Array.from({ length: 10 }, (_, i) => `Opt${i}`);
  const rows = buildPollComponents("g_10", opts);
  assertEquals(rows.length, 2);
  assertEquals(rows[0].components.length, 5);
  assertEquals(rows[1].components.length, 5);
});
