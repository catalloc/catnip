...
// --- Batch 6b: additional coverage ---

Deno.test("xp: negative addXp does not decrease total", async () => {
  resetStore();
  // Grant 200 XP first
  await xp.grantXp("g1", "u_neg", 200);
  // Grant negative XP — the code does s.xp += amount, so it will decrease
  // but totalXpEarned also increases by negative amount
  const result = await xp.grantXp("g1", "u_neg", -50);
  // XP decreases (no guard in code)
  assertEquals(result.state.xp, 150);
  // totalXpEarned also changed (150) — code adds amount to totalXpEarned
  assertEquals(result.state.totalXpEarned, 150);
});

Deno.test("xp: level 0 boundary has 0 XP requirement", () => {
  assertEquals(xpForLevel(0), 0);
  // Also verify that negative levels return 0
  assertEquals(xpForLevel(-5), 0);
});

Deno.test("xp: totalXpForLevel matches cumulative xpForLevel", () => {
  // Verify totalXpForLevel(5) === sum of xpForLevel(1) + ... + xpForLevel(5)
  let sum = 0;
  for (let i = 1; i <= 5; i++) {
    sum += xpForLevel(i);
  }
  assertEquals(totalXpForLevel(5), sum);

  // Also verify for level 10
  let sum10 = 0;
  for (let i = 1; i <= 10; i++) {
    sum10 += xpForLevel(i);
  }
  assertEquals(totalXpForLevel(10), sum10);
});

Deno.test("xp: large XP value computes correct level", () => {
  const level = levelFromXp(1_000_000);
  assert(Number.isFinite(level));
  assert(level > 0);
  // At 1M XP the level should be substantial
  assert(level > 10, `Expected level > 10, got ${level}`);
  // Verify round-trip: totalXpForLevel(level) <= 1M < totalXpForLevel(level+1)
  assert(totalXpForLevel(level) <= 1_000_000);
  assert(totalXpForLevel(level + 1) > 1_000_000);
});

Deno.test("xp: new user getOrCreate returns level 0", async () => {
  resetStore();
  const state = await xp.getOrCreate("g99", "u99");
  assertEquals(state.level, 0);
  assertEquals(state.xp, 0);
  assertEquals(state.totalXpEarned, 0);
  assertEquals(state.guildId, "g99");
  assertEquals(state.userId, "u99");
});
