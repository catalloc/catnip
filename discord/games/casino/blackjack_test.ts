...
// --- Batch 6a: additional coverage ---

Deno.test("blackjack: determineOutcome player bust returns lose", () => {
  // Player has 3 face cards = 30, clearly bust
  const session = {
    playerHand: [makeCard("K"), makeCard("J"), makeCard("Q")], // 30
    dealerHand: [makeCard("9"), makeCard("8")], // 17
  } as any;
  assertEquals(determineOutcome(session), "player-bust");
});

Deno.test("blackjack: determineOutcome dealer bust returns win", () => {
  // Player stands at 15, dealer busts at 26
  const session = {
    playerHand: [makeCard("7"), makeCard("8")], // 15
    dealerHand: [makeCard("K"), makeCard("J"), makeCard("6")], // 26
  } as any;
  assertEquals(determineOutcome(session), "dealer-bust");
});

Deno.test("blackjack: determineOutcome push when equal", () => {
  // Both have 20
  const session = {
    playerHand: [makeCard("Q"), makeCard("K")], // 20
    dealerHand: [makeCard("J"), makeCard("10")], // 20
  } as any;
  assertEquals(determineOutcome(session), "push");
});

Deno.test("blackjack: calculatePayout blackjack returns 2.5x bet", () => {
  // 200 * 2.5 = 500
  assertEquals(calculatePayout(200, "player-blackjack"), 500);
  // 77 * 2.5 = 192.5, floored to 192
  assertEquals(calculatePayout(77, "player-blackjack"), Math.floor(77 * 2.5));
});

Deno.test("blackjack: calculatePayout push returns 1x bet", () => {
  assertEquals(calculatePayout(50, "push"), 50);
  assertEquals(calculatePayout(999, "push"), 999);
});
