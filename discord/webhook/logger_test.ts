...
// --- Batch 3b: additional tests ---

Deno.test("sanitize: multiple tokens in same string all redacted", () => {
  const input = "Bearer tok1.abc.def and Bot tok2.xyz.ghi";
  const result = sanitize(input);
  assert(result.includes("Bearer [REDACTED]"), "Should redact Bearer token");
  assert(result.includes("Bot [REDACTED]"), "Should redact Bot token");
  assert(!result.includes("tok1"), "First token should be gone");
  assert(!result.includes("tok2"), "Second token should be gone");
});

Deno.test("sanitize: partial match does not corrupt text", () => {
  const input = "Bearish market trends are down";
  const result = sanitize(input);
  assertEquals(result, input);
});

Deno.test("logger: log at debug when minLevel is info is ignored", async () => {
  const logger = createBufferLogger({ minLevel: "info" });
  logger.debug("this should be ignored");
  // With webhookUrl: null, flush just clears the buffer.
  // If the debug message were accepted, buffer would have 1 entry.
  // We verify indirectly by ensuring finalize completes without sending anything.
  await logger.finalize();
  // No error and no side effects means the debug message was properly filtered.
});

Deno.test("logger: error with Error.cause property does not crash", () => {
  const messages: string[] = [];
  const original = console.log;
  console.log = (msg: string) => messages.push(msg);
  try {
    ...
    assert(messages.length > 0, "Should have logged something");
    assert(messages[0].includes("wrapper error"), "Should include the wrapper error message");
  } finally {
    console.log = original;
  }
});

Deno.test("logger: finalize called twice is safe", async () => {
  const logger = createBufferLogger();
  logger.info("msg");
  await logger.finalize();
  // Second finalize should be a no-op and not throw
  await logger.finalize();
});

Deno.test("logger: multiple loggers all finalize via finalizeAllLoggers", async () => {
  const sent: string[] = [];
  const originalFetch = globalThis.fetch;
  globalThis.fetch = (_input: string | URL | Request, init?: RequestInit) => {
    sent.push(init?.body as string);
  ...
  try {
    const logger1 = createLogger("Multi1", {
      webhookUrl: "https://discord.com/api/webhooks/test/token",
      fallbackToConsole: false,
      batchIntervalMs: 999_999,
    });
    const logger2 = createLogger("Multi2", {
      webhookUrl: "https://discord.com/api/webhooks/test/token",
      fallbackToConsole: false,
      batchIntervalMs: 999_999,
    });
    ...
    await finalizeAllLoggers();
    // Both loggers should have flushed
    assert(sent.length >= 2, `Expected at least 2 sends, got ${sent.length}`);
    const allContent = sent.join(" ");
    assert(allContent.includes("from logger1"), "Should contain logger1 message");
    assert(allContent.includes("from logger2"), "Should contain logger2 message");
  } finally {
    globalThis.fetch = originalFetch;
  }
});

Deno.test("logger: batch format includes context header", async () => {
  const sent: string[] = [];
  const originalFetch = globalThis.fetch;
  globalThis.fetch = (_input: string | URL | Request, init?: RequestInit) => {
    sent.push(init?.body as string);
  ...
  try {
    ...
    await logger.finalize();
    assert(sent.length > 0, "Should have sent");
    const payload = JSON.parse(sent[0]);
    const content = payload.content as string;
    assert(content.includes("MyContext"), "Batch header should include the context name");
  } finally {
    globalThis.fetch = originalFetch;
  }
});
